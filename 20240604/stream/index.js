...
// 청크의 크기
// 스트림에서 작업할 때 데이터를 받고 처리할 때마다 각 청크를 조작한다.

const chunkSize = 64 * 1024; // 64KB

// 스트림으로 파일의 데이터를 읽어보자
// 스트림 데이터를 읽어 온다. 매개변수로 파일의 경로 주기
// highWaterMark : 청크의 크기를 설정해서 읽는 스트림을 만든다.
const test1 = fs.createReadStream("test1.txt", { highWaterMark: chunkSize });
console.log(test1);
...
test1.pipe(transformData).pipe(test2);
