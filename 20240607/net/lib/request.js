...
const getHeaderEndIndex = (request) => request.indexOf(GUILOTINE);
...
const getRequest = (buffer) => {
    ...
    const [headerString, ...bodyString] = buffer.toString().split(GUILOTINE);
    const body = bodyString.join(GUILOTINE); // body 문자열로 변환

    const header = getHeaders(headerString); // header 객체 생성
    console.log(header);
    // body 내용 있을 때 객체로 만들자
    return {
        ...header,
        body
    }
